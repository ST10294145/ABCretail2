...
namespace ABCretail2.Services
{
    public class AzureFileShareService
    {
        ...
        {
            _serviceClient = new ShareServiceClient(connectionString ?? throw new ArgumentNullException(nameof(connectionString)));
        ...
        public async Task UploadFileAsync(string directoryName, string fileName, Stream fileStream)
        {
            ...
            try
            {
                var shareClient = _serviceClient.GetShareClient(_fileShareName);
